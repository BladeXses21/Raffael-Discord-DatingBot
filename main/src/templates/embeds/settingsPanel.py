...
from config import emoji_font, emoji_age, emoji_back, emoji_description, emoji_games, emoji_gender, emoji_opposite_gender, emoji_location, \
    emoji_location, emoji_name, emoji_photo
...
class SettingsEmbed(object):
    def __init__(self, user_language):
        ...
        self._embed.add_field(value=f"{emoji_opposite_gender}{emoji_font}—{emoji_font}{translate_text('edit_opposite_gender', user_language)}", name="", inline=True)
        self._embed.add_field(name="", value="ㅤ", inline=True)
        self._embed.add_field(value=f"{emoji_location}{emoji_font}—{emoji_font}{translate_text('edit_location', user_language)}", name="", inline=True)
        self._embed.add_field(value=f"{emoji_games}{emoji_font}—{emoji_font}{translate_text('edit_game', user_language)}", name="", inline=True)
        self._embed.add_field(name="", value="ㅤ", inline=True)
        self._embed.add_field(value=f"{emoji_description}{emoji_font}—{emoji_font}{translate_text('edit_description', user_language)}", name="", inline=True)
